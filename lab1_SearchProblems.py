...
def cell_to_GVD_gradient_ascent(
    ...
    while cell not in GVD:
        nbs = neighbors(grid, cell[0], cell[1])
        max_val = -1
        for next in nbs:
            if grid[next[0]][next[1]] > max_val:
                max_val = grid[next[0]][next[1]]
                max_next = next
        cell = max_next
        path.append(cell)
...
def cell_to_GVD_a_star(
    ...
    while not frontier.empty():
        ...
        curr = frontier.get()[1]
        curr_depth = reached[curr]["cost"]
        if curr in GVD:
            break
        for child in neighbors(grid, curr[0], curr[1]):
            f = curr_depth + 1 + distance(child, goal)
            if child not in reached.keys() or reached[child]["cost"] > curr_depth + 1:
                reached[child] = {"cost": curr_depth + 1, "parent": curr}
...
def GVD_path(
    ...
    while len(frontier) > 0:
        ...
        if mode == PathPlanMode.DFS:
            ...
            curr = frontier.pop(0)
        for child in neighbors(grid, curr[0], curr[1]):
            if child in GVD and child not in closed:
                pointers[child] = curr
                if child[0] == B[0] and child[1] == B[1]:
                    path = []
                    while child is not None:
                        path.insert(0, child)
                        if child not in pointers.keys():
                            break
